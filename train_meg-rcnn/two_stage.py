...
class TwoStageDetector(BaseDetector):
    ...
    def loss(self, batch_inputs: Tensor,
        ...
        id0 = torch.zeros_like(rpn_results_list[0].bboxes[:, 0]).unsqueeze(1)
        bboxes0 = torch.cat([id0, rpn_results_list[0].bboxes], dim=1)
        id1 = torch.ones_like(rpn_results_list[1].bboxes[:, 0]).unsqueeze(1)
        bboxes1 = torch.cat([id1, rpn_results_list[1].bboxes], dim=1)
        _rpn_results_list = torch.cat([bboxes0, bboxes1], dim=0)
        ...
        gt_boxes = torch.zeros([2, 500, 5], dtype=torch.float32).to(_boxes[0].device)
        gt_boxes[0, :im_info[0], :4] = _boxes[0]
        gt_boxes[0, :im_info[0], 4] = 1
        gt_boxes[1, :im_info[1], :4] = _boxes[1]
        gt_boxes[1, :im_info[1], 4] = 1
        roi_losses = self.roi_meg(x, _rpn_results_list, im_info, gt_boxes, top_k=1)
    ...
    def rpn_meg(self, features, rpn_data_samples):
        ...
        localization_loss = smooth_l1_loss(pred_bbox_offsets[pos_masks], rpn_bbox_targets[pos_masks],
                                           1)  # config.rpn_smooth_l1_beta
    ...
    def roi_meg(self, x, rpn_rois, im_info, gt_boxes, top_k=1):
        rcnn_rois, rcnn_labels, rcnn_bbox_targets = fpn_roi_target(rpn_rois, im_info, gt_boxes, top_k=top_k)

        fpn_fms = x[:4]
        stride = [4, 8, 16, 32]
        pool_features = roi_pooler(fpn_fms, rcnn_rois, stride, (7, 7), "ROIAlignV2")
        flatten_feature = torch.flatten(pool_features, start_dim=1)
        flatten_feature = F.relu_(self.roi_head.bbox_head.shared_fcs[0](flatten_feature))
        flatten_feature = F.relu_(self.roi_head.bbox_head.shared_fcs[1](flatten_feature))
        pred_cls = self.roi_head.bbox_head.fc_cls(flatten_feature)
        pred_delta = self.roi_head.bbox_head.fc_reg(flatten_feature)

        # loss for regression
        labels = rcnn_labels.long().flatten()
        fg_masks = labels > 0
        valid_masks = labels >= 0

        pred_delta = pred_delta[fg_masks]
        localization_loss = smooth_l1_loss(
            pred_delta, rcnn_bbox_targets[fg_masks], 1)  # config.rcnn_smooth_l1_beta
        # loss for classification
        _labels = torch.ones_like(labels)
        _labels[fg_masks] = 0
        objectness_loss = softmax_loss(pred_cls, _labels)
        ...
        loss_dict = {}
        loss_dict['loss_bbox'] = loss_rcnn_loc
        loss_dict['loss_cls'] = loss_rcnn_cls
        return loss_dict
...
def fpn_roi_target(rpn_rois, im_info, gt_boxes, top_k=1):
    ...
    for bid in range(2):  # config.train_batch_per_gpu
        gt_boxes_perimg = gt_boxes[bid, :int(im_info[bid]), :]
        ...
        # cons masks
        ignore_assign_mask = (max_overlaps_normal < 0.5) * (max_overlaps_ignore > max_overlaps_normal)  # config.fg_threshold
        ...
        fg_mask = (max_overlaps >= 0.5) * (labels != -1)  # config.fg_threshold config.ignore_label
        bg_mask = (max_overlaps < 0.5) * (max_overlaps >= 0)  # config.bg_threshold_high config.bg_threshold_low
        fg_mask = fg_mask.reshape(-1, top_k)
        bg_mask = bg_mask.reshape(-1, top_k)
        pos_max = 125  # config.num_rois * config.fg_ratio
        fg_inds_mask = subsample_masks(fg_mask[:, 0], pos_max, True)
        neg_max = 512 - fg_inds_mask.sum()  # config.num_rois
        ...
        std_opr = torch.tensor([0.1, 0.1, 0.2, 0.2]).type_as(bbox_targets)  # bbox_normalize_stds
        mean_opr = torch.tensor([0, 0, 0, 0]).type_as(bbox_targets)  # bbox_normalize_means
    ...
    if 2 == 1:  # config.train_batch_per_gpu
...
def fpn_anchor_target(boxes, im_info, all_anchors_list):
    ...
    for bid in range(2):  # config.train_batch_per_gpu
        ...
        # sample labels
        pos_idx, neg_idx = subsample_labels(concated_batch_labels, 256,
                                            0.5)  # config.num_sample_anchors, config.positive_anchor_ratio
